...
final class ActionMapping {
	...
	void buildActionMapping() {
		...
		for (Entry<String, Class<? extends Controller>> entry : routes.getEntrySet()) { // 循环已注册路由
			Class<? extends Controller> controllerClass = entry.getValue(); // 控制器类
			...
			for (Method method : methods) { // 循环处理方法
				...
				Interceptor[] actionInters = interMan.buildControllerActionInterceptor(controllerInters, controllerClass, method); // action级拦截器
				String controllerKey = entry.getKey(); // 路由注册controllerKey
